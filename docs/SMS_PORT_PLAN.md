...
| Surveys | Yes/no branch targets. Retry, timeout, and session TTL on create. Live preview of the invitation. Test mode sends only to an org test roster (cap 25). Answer table and wide CSV (parsed value and verbatim). Create a list from completed, started, or non-responders. Archive after close. |
